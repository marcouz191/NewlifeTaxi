...
AddEventHandler('VerouillerLeVehiculeMenu', function()
	local pos = GetEntityCoords(GetPlayerPed(-1),true)
	Citizen.Wait(100)
	print(pos)
	local localVehId = ESX.Game.GetClosestVehicle(pos)
	Citizen.Wait(100)
	print(localVehId)
	print(GetVehicleEngineHealth(localVehId))
    if(localVehId and localVehId ~= 0)then
        local localVehPlateTest = GetVehicleNumberPlateText(localVehId)
        if localVehPlateTest ~= nil then
            local localVehPlate = string.lower(localVehPlateTest)
...
AddEventHandler("ls:getHasOwner2", function(hasOwner, localVehId, localVehPlate, localVehLockStatus)
    TriggerEvent("ls:newVehicle", localVehPlate, localVehId, localVehLockStatus)
    TriggerServerEvent("ls:addOwner", localVehPlate)
    TriggerEvent("ls:notify", getRandomMsg())
end)
...
function Notify(text, duration)
	if(Config.notification)then
		if(Config.notification == 1)then
			...
			Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_PEGASUS_DELIVERY", "CHAR_PEGASUS_DELIVERY", true, 1, "Système de clé" .. _VERSION, "OriginalBigCityQc", duration)
